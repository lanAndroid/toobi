...
public class Inform2Adapter extends Adapter<ViewHolder> {
    private Context context;
    private List<roomSearch.DataBean> inform;

    public Inform2Adapter(Context context,List<roomSearch.DataBean> inform) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        MyHolder myHolder;
        if (viewType == 1) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.inform_item, parent, false);
            view.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT));
            myHolder = new MyHolder(view);
        } else {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.inform_item2, parent, false);
            view.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT));
            myHolder = new MyHolder(view);
        }
        return myHolder;
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        if (myHolder.room_name1 != null && myHolder.room_pic1 != null && myHolder.room_suo != null) {
            Picasso.with(context).load(inform.get(position).getRoom_pic()).into(myHolder.room_pic1);
            myHolder.room_name1.setText(inform.get(position).getRoom_name());
        }
        if (myHolder.room_name2 != null && myHolder.room_pic2 != null) {
            Picasso.with(context).load(inform.get(position).getRoom_pic()).into(myHolder.room_pic2);
