...
public class CaptureVideoActivity extends UI implements SurfaceHolder.Callback {
    ...
    private void switchCamera() {
        if (Build.VERSION.SDK_INT >= 9) {
            cameraId = (cameraId + 1) % android.hardware.Camera.getNumberOfCameras();
        }
        resizeSurfaceView();
        shutdownCamera();
        initCamera();
    ...
    private void setCameraParameters() {
        ...
        if (focusMode.contains(android.hardware.Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO)) {
    ...
    @SuppressWarnings("deprecation")
    @SuppressLint("NewApi")
    public int setCameraDisplayOrientation(Context context, int cameraId, android.hardware.Camera camera) {
        int orientation = 90;
        ...
        if (Build.VERSION.SDK_INT >= 9) {
            ...
            front = (info.facing == android.hardware.Camera.CameraInfo.CAMERA_FACING_FRONT);
        }
