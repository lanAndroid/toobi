...
public class UserInfoHelper {
    ...
    public static String getUserTitleName(String id, SessionTypeEnum sessionType) {
        if (sessionType == SessionTypeEnum.P2P) {
            ...
            return TeamDataCache.getInstance().getTeamName("");
        }
        return id;
    }

    public static String getUserTitleName(String id, SessionTypeEnum sessionType, String name) {
        if (sessionType == SessionTypeEnum.P2P) {
            if (NimUIKit.getAccount().equals(id)) {
                return "";
            } else {
                return NimUserInfoCache.getInstance().getUserDisplayName(id, name);
            }
        } else if (sessionType == SessionTypeEnum.Team) {
            return TeamDataCache.getInstance().getTeamName("");
        }
